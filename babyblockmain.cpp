...
int main(void)
{
	...
	cout << "Welcome to the baby block robot! This robot will accurately and efficiently sort " << "20 random letters A-Z one at a time. ";
	...
	while (blocks_placed < 20) {
		if (current_block == ' ')
			current_block = get_block(); // Get a block if the robot isn't currently holding one
	...
	cout << "All blocks have been placed in the correct order! Thank you for using the baby block robot. " 
		 << swap_count << " swaps were required in this sorting." << endl << endl;
